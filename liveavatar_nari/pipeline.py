...
class WanS2V:
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device, kv_cache_size=13500):
        ...
        self.kv_cache = [
            {
                "k": torch.zeros(
                    [batch_size, kv_cache_size, nh, hd],
                    dtype=dtype,
                    device=cache_device,
                ),
                "v": torch.zeros(
                ...
                "cond_k": torch.zeros(
                    [batch_size, self.dit_max_cond_cache_tokens, nh, hd],
                    dtype=dtype,
                    device=cache_device,
                ),
                "cond_v": torch.zeros(
        ...
        prefill_kv = [
            {
                "k": layer["k"][0:1],
                "v": layer["v"][0:1],
                "cond_k": layer["cond_k"][0:1],
                "cond_v": layer["cond_v"][0:1],
    ...
    def _prepare_image(self, ref_image_path, max_area):
        ...
        HEIGHT, WIDTH = self.get_size_less_than_area(ref_image.shape[0], ref_image.shape[1], target_area=max_area)
    ...
    def generate(
        ...
        ref_image_latents, motion_latents, motion_pixel_frames, HEIGHT, WIDTH = self._prepare_image(
