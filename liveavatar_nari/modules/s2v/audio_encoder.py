...
class AudioEncoder:
    ...
    def extract_audio_feat(self, audio_path, return_all_layers=False, dtype=torch.float32):
        ...
        res = self.model(
            input_values.to(device=self.model.device, dtype=self.model.dtype),
    ...
    def extract_audio_feat_from_array(
        self,
        audio_array,
        sample_rate=16000,
        ...
        """Extract audio features from a numpy array (no file path needed)."""
        ...
        # retrieve logits & take argmax
        res = self.model(
            input_values.to(device=self.model.device, dtype=torch.bfloat16),
            output_hidden_states=True,
        )
        if return_all_layers:
            feat = torch.cat(res.hidden_states)
