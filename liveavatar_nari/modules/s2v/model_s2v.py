...
def sp_attn_forward_s2v(self, x, seq_lens, grid_sizes, freqs, dtype=torch.bfloat16, sp_size=None):
...
class Head_S2V(Head):
    def forward(self, x, e):
        ...
        with amp.autocast(dtype=torch.float32):
            e = (self.modulation + e.unsqueeze(1)).chunk(
                2, dim=1
            )  # modulation:nn.Parameter(torch.randn(1, 2, dim) / dim**0.5)  +[b,1,dim] ->[b,2,dim]->chunk->tuple(2)*[b,1,dim]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.blocks = nn.ModuleList(
            [
                WanS2VAttentionBlock(dim, ffn_dim, num_heads, window_size, qk_norm, cross_attn_norm, eps)
    ...
    def forward(
        ...
        **extra_kwargs,
        ...
        if self.enbale_adain:
            ...
            self.audio_emb_global = audio_emb_global[:, motion_frames[1] :].clone().to(dtype=torch.bfloat16)
        else:
        ...
        if self.zero_timestep:
            t = torch.cat([t, torch.zeros([1], dtype=t.dtype, device=t.device)])  # [b]->[b+1], default is True
        with amp.autocast(dtype=torch.float32):
            e = self.time_embedding(sinusoidal_embedding_1d(self.freq_dim, t).float())  # t:[b+1],output:[b+1,dim]
