#!/bin/bash
# Streaming pipeline — per-block VAE decode + on-demand audio

CUDA_VISIBLE_DEVICES=0 python minimal_inference/s2v_2gpu_streaming.py \
    ...
    --sample_steps 4 \
    --seed 420 \
    --output output/result_streaming.mp4
